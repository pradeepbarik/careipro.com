...
const StandardBannerAd = ({ page_type, category_ids, city, limit = 1, showPlaceholder = false, ad_selection = 'random' }: StandardBannerAdProps) => {
    const { ads, loading, error, containerRef, handleAdClick } = useAds({
        page_type,
        category_ids,
        city,
        limit,
        asp_ratio: '4:3', // Standard banner aspect ratio
        ad_selection,
    });
    ...
    return (
        <div ref={containerRef} className="w-full my-2">
            {loading && (
                <div className="w-full bg-gray-100 animate-pulse rounded-lg" style={{ aspectRatio: '4/3' }} />
            )}
...
export default StandardBannerAd;
